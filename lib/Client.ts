...
import type {  ClientInstanceOptions, ClientOptions, Handlers } from "./types/client";
...
import type { LoggedInUserData } from "@rbxdiscord/roblox";
...
export default class Client<E extends ClientEvents = ClientEvents> extends TypedEmitter<E> {
    ...
    private _robloxUser?: LoggedInUserData;
    ...
    folderData?: { commands: string, events: string }
    ...
    async loadRoblox() {
        this.robloxClient = new RobloxClient(this.options.cookie!)
        const user = await this.robloxClient.start();
        this._robloxUser = user;
        return user;
    }

    /** The client's roblox user. This will throw an error if not using a gateway connection or no shard is READY. */
    get robloxUser() : LoggedInUserData {
        if (this._robloxUser) {
            return this._robloxUser;
        } else {
